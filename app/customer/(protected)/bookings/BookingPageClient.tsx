...
const getSeatStatus = (seed: string, row: number, column: number) => {
  const hash = hashString(`${seed}:${row}:${column}`);
  return hash % 10 > 6 ? "BOOKED" : "AVAILABLE";
};

const generateSeats = (seed: string): Seat[] => {
  const rows = 8;
  const columns = 12;
  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let c = 0; c < columns; c++) {
      const status = getSeatStatus(seed, r, c);
...
function BookingPageClient({ showtime, onBack }: BookingPageProps) {
  ...
  const seats = useMemo(
    () => generateSeats(currentShowtime.id || currentShowtime.movieTitle),
    [currentShowtime.id, currentShowtime.movieTitle],
  );
  ...
  const handleMouseUp = () => {};
  const handleContextMenu = (_seat: Seat, event: React.MouseEvent) => {
    event.preventDefault();
  };

  const getSelectedSeatsDetails = () => seats.filter((seat) => selectedSeats.has(seat.id));
  const getSelectedSeatLabels = () =>
    getSelectedSeatsDetails().map((seat) => `${seat.row}${seat.seatNumber ?? seat.column + 1}`);
  const getTotalAmount = () =>
    getSelectedSeatsDetails().reduce(
      (total, seat) => total + (seat.seatType === "VIP" ? 25 : 15),
      0,
    );
